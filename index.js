...
>>>>>>> 2f794b060671c8359847d68c2ffb5248398887af
...
async function logData(message) {
    const user = await client.dbInstance
        .collection("users")
        .findOne({ uuid: message.author.id });
    if (user == null) {
...
async function databaseConnect() {
    databaseClient = await new MongoClient(config.databaseURL, {
...
<<<<<<< HEAD
let replies = {
    //autoreply system based on keywords
};
...
async function isBad(message) {
    ...
    if (
        swearjar.profane(messageString) &&
    ...
    try {
        if (command.guildOnly && message.channel.type !== "GUILD_TEXT") {
            return message.reply(
                "I can't execute that command inside DMs or threads!"
            );
        }
        if (command.args && !args.length) {
            ...
            if (command.needsmod) {
                ...
                modRoles.forEach((element) => {
                    if (
                        currentMember.roles.cache.has(element) ||
                        currentMember.permissions.has(["ADMINISTRATOR"])
                    ) {
                        isMod = true;
                    }
                    if (isMod) return;
